...
function checkPriceInputs() {
  if (
    isNaN(minPriceValue.value) ||
    isNaN(maxPriceValue.value) ||
    ...
    priceInputErrorMessage.value =
      'Minimum value cannot be larger than \nor equal to Maximum value';
    minPriceValue.value = '1';
    maxPriceValue.value = '1000';
  } else {
    priceInputErrorMessage.value = '';
...
// applys final filters and reducing the dataset to display
function filterResults() {
  ...
  if (searchTerm.value != '') {
    for (let i = 0; i < variables.saleItems.length; i++) {
      ...
      let searchUpper = Object.keys(current).reduce(
        (accu, curr) => ({
          ...accu,
          [curr]: typeof current[curr] === 'string' ? current[curr].toUpperCase() : current[curr]
        }),
        {}
      );

      if (
        Object.values(searchUpper).some(
          (check) => typeof check === 'string' && check.includes(searchTerm.value.toUpperCase())
        )
      ) {
  ...
  newResults = newResults.filter((item) =>
    categoriesSelected.value.some(
      (cat) => item.Categories.toLowerCase().trim() === cat.toLowerCase().trim()
    )
  );

  for (const item of newResults) {
    if (
      item.Categories == 'Enclosures' &&
...
<template>
  <button
    class="btn btn-outline-secondary w-100 d-lg-none show-filters filtersBorder"
    type="button"
    data-bs-toggle="collapse"
    data-bs-target="#saleFilters"
    aria-controls="saleFilters"
    aria-expanded="false"
  ...
  <div class="collapse card d-lg-block mb-5" id="saleFilters">
    <div class="accordion filtersBorder" id="saleFiltersSide">
      <button
        type="button"
        class="btn btn-white w-50 border border-secondary"
        @click="applyAllFilters(), $emit('updateItemsFound', itemsToFilter), $emit('showPerPage')"
      >
        Apply & Search
      </button>
      <button
        type="button"
        class="btn btn-white w-50 border border-secondary"
        @click="clearAllFilters(), $emit('updateItemsFound', itemsToFilter), $emit('showPerPage')"
      >
        Clear All Filters
      </button>
      <!-- categories filter -->
      <div class="accordion-item">
        ...
        <div
          ...
          aria-labelledby="categoriesButton"
        >
          <div class="accordion-body">
            <div class="form-check">
              <input
                ...
                checked
              />
              <label class="form-check-label" for="AccessoriesCheck"> Accessories </label>
            </div>
            <div class="form-check">
              <input
                class="form-check-input categoryCheck"
            ...
            <div class="form-check">
              <input
                ...
                checked
              />
              <label class="form-check-label" for="EnclosuresCheck"> Enclosures </label>
            </div>
            <div class="form-check">
              <input
                class="form-check-input categoryCheck"
                type="checkbox"
                value=""
                id="HeatAndLightCheck"
                checked
              />
              <label class="form-check-label" for="HeatAndLightCheck"> Heat and Light </label>
            </div>
            <div class="form-check">
              <input
                class="form-check-input categoryCheck"
      ...
      <div class="accordion-item">
        ...
        >
          <div class="accordion-body">
            ...
            <div class="form-check">
              <input
                class="form-check-input sizeCheck"
                type="checkbox"
                value=""
                id="LargeCheck"
                checked
            ...
            <div class="form-check">
              <input
                ...
                checked
              />
              <label class="form-check-label" for="SmallCheck"> Small </label>
            </div>
            Height
            <div class="form-check">
              <input
                ...
                checked
              />
              <label class="form-check-label" for="TallCheck"> Tall </label>
            </div>
            <div class="form-check">
              <input
                class="form-check-input sizeCheck"
                ...
                checked
              />
              <label class="form-check-label" for="AverageCheck"> Average </label>
            </div>
            <div class="form-check">
              <input
                class="form-check-input sizeCheck"
              ...
              <div class="col-6">
                ...
                <div class="form-outline">
                  ...
                  <label class="form-label" for="typeNumber">{{
                    (1).toLocaleString(undefined, { style: 'currency', currency: currencyType })
                  }}</label>
                </div>
              </div>
              <div class="col-6">
                ...
                <div class="form-outline">
                  ...
                  <label class="form-label" for="typeNumber">{{
                    (1000 * currencyRate).toLocaleString(undefined, {
                      style: 'currency',
