...
//when page is mounted, adds items to recommendation list
onMounted(async () => {
  try {
  ...
  const recQuery = await getDocs(
    ...
    where('Categories', '==', "'" + item.value.Categories + "'"),
    limit(3)
  );
  let recItems = [];
...
//sets minimum and maximum quantity of items
let quant = ref(1);
...
// Checks if item is already in users basket. Adds it if not.
// let openLoginBasketAlert1 = false;
const addOrIn = ref('');
const emit = defineEmits(['loginBasketAlertApp', 'addedBasketAlertApp', 'inBasketAlertApp']);
async function addToBasket() {
  if (variables.user) {
    ...
    function addBasket() {
      basket.push(variables.id + ' /::' + String(1));
      updateDoc(userRef, 'Basket', basket);
      emit('addedBasketAlertApp');
    }
  } else {
    emit('loginBasketAlertApp');
  }
}
</script>
          ...
          <div class="row mb-4">
            <div class="col-md-4 col-6 mb-3">
              ...
              <select
                ...
                v-model="chosenColour"
                style="height: 35px; width: 120px"
              >
                <option value="Black">Black</option>
                <option v-if="item.Categories == 'Enclosures'" value="Wood-Brown">
                  Wood-Brown
                </option>
                <option v-if="item.Categories == 'Enclosures'" value="White">White</option>

                <option v-if="item.Categories == 'Basking and Hydrating'" value="Natural Rock">
                  Natural Rock
                </option>
                <option v-if="item.Categories == 'Basking and Hydrating'" value="Mossy Log">
                  Mossy Log
                </option>
                <option v-if="item.Categories == 'Basking and Hydrating'" value="Sandstone">
                  Sandstone
                </option>

                <option v-if="item.Categories == 'Bedding and Substrate'" value="Sandy">
                  Sandy
                </option>
                <option v-if="item.Categories == 'Bedding and Substrate'" value="Dirty">
                  Dirty
                </option>
                <option v-if="item.Categories == 'Bedding and Substrate'" value="Dark Shaving">
                  Dark Shaving
                </option>
              </select>
            </div>
            ...
            <div class="col-md-4 col-6 mb-3">
              ...
              <div class="input-group mb-3 border-success" style="width: 120px">
                <input
                  ...
                  :value="
                    (item.Price * currencyRate * quant).toLocaleString(undefined, {
                      style: 'currency',
                      currency: currencyType
                    })
                  "
            ...
            <div class="col-md-7">
              <router-link
                ...
                :to="{
                  name: 'Payment',
                  params: { id: item.StripePriceId.substring(6), quantity: quant }
                }"
              >
                <button class="btn btn-success">Buy Now</button>
              </router-link>
              <a class="btn btn-primary" style="margin-right: 5px" @click="addToBasket()">
                Add to Basket
              </a>
            </div>
                  ...
                  <div class="d-flex flex-row">
                    <h5 class="mb-1 me-1">
                      <router-link
                        ...
                        >{{ recommendation.Title }}</router-link
                      >
                    </h5>
                    <br />
                    <h6>
                      {{
                        (recommendation.Price * currencyRate).toLocaleString(undefined, {
                          style: 'currency',
                          currency: currencyType
                        })
                      }}
